export const arrayOfStringsHasEntriesWithSpaces = (values: string[]): boolean => values.filter(value => stringContainsSpaces(value)).length > 0;

export const arrayOfStringsDoesNotHaveEntriesWithSpaces = (values: string[]): boolean => !arrayOfStringsHasEntriesWithSpaces(values);
...
export const isArrayOfBooleans = (values: unknown[]): boolean => isArray(values) && (values).every((value: unknown) => (typeof value === 'boolean' && isBoolean(value)));

export const isNotArrayOfBooleans = (values: unknown[]): boolean => !isArrayOfBooleans(values);

export const isArrayOfNumbers = (values: unknown[]): boolean => isArray(values) && (values).every((value: unknown) => (typeof value === 'number' && isNumber(value)));

export const isNotArrayOfNumbers = (values: unknown[]): boolean => !isArrayOfNumbers(values);

export const isArrayOfStrings = (values: unknown[]): boolean => isArray(values) && (values).every((value: unknown) => (typeof value === 'string' && value !== ''));
...
export const isEmptyArray = (values: unknown[]): boolean => values.length === 0;

export const isNotEmptyArray = (values: unknown[]): boolean => !isEmptyArray(values);
